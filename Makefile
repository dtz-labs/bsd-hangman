...
# CI prepares Python-generated sources on the Ubuntu runner, then invokes
# `make tapes` inside the official z88dk image (which does not ship Python).
...
pot: $(GETTEXT_MESSAGES)
	...
	xgettext --language=C --from-code=UTF-8 \
		...
		--package-name='ZX Spectrum Hangman' --package-version=1.0 \
