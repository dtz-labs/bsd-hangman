...
def validate_strings(strings: dict[str, str]) -> None:
    ...
    if not strings["license_key"].startswith("L"):
        raise ValueError("license_key must use L to open the licence screen")
    if "A-Z" not in strings["ascii_notice"]:
        raise ValueError("ascii_notice must explicitly state the A-Z limitation")
    for key in ("round_label", "wins_label", "dictionary_label"):
...
def build(
    ...
    lines = [
        ...
        "#ifndef HANGMAN_LOCALE_H",
        "#define HANGMAN_LOCALE_H",
        "",
